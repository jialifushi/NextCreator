...
import { Box, Play, AlertCircle, Maximize2, CircleAlert } from "lucide-react";
import { useFlowStore } from "@/stores/flowStore";
import { useCanvasStore } from "@/stores/canvasStore";
import { generateImage } from "@/services/imageGeneration";
import { saveImage, getImageUrl } from "@/services/fileStorageService";
...
// Z-Image 节点数据类型
interface ZImageGeneratorNodeData {
  [key: string]: unknown;
  label: string;
  model: ModelType;
...
type ZImageGeneratorNode = Node<ZImageGeneratorNodeData>;

// 预设模型选项
const presetModels = [
  { value: "z-image-turbo", label: "Z-Image Turbo" },
  { value: "z-image-ultra", label: "Z-Image Ultra" },
...
function ZImageGeneratorBase({
  ...
  // 默认模型
  const defaultModel: ModelType = "z-image-turbo";
  ...
  const handleModelChange = (value: string) => {
    updateNodeData<ZImageGeneratorNodeData>(id, { model: value });
  };

  // 更新节点数据，同时更新 canvasStore
  const updateNodeDataWithCanvas = useCallback((nodeId: string, nodeData: Partial<ZImageGeneratorNodeData>) => {
    const { activeCanvasId } = useCanvasStore.getState();
    const targetCanvasId = canvasIdRef.current;

    updateNodeData<ZImageGeneratorNodeData>(nodeId, nodeData);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      // Z-Image 不支持参考图，直接使用文生图
      const response = await generateImage({
        prompt,
        model,
        aspectRatio: data.aspectRatio,
        negativePrompt: negativePrompt || undefined, // 传递负面提示词
      }, "zImageGenerator");
  ...
  }, [id, model, data.aspectRatio, negativePrompt, updateNodeDataWithCanvas, getConnectedInputDataAsync]);
  ...
  return (
    <>
      <div
        ...
        `}
      >
        {/* 输入端口 - prompt（Z-Image 不支持参考图，只有提示词输入） */}
        <Handle
          type="target"
        ...
        {/* 节点头部 - indigo 渐变 */}
        <div className="flex items-center justify-between px-3 py-2 bg-gradient-to-r from-indigo-500 to-blue-500 rounded-t-lg">
        ...
        <div className="p-2 space-y-2 nodrag">
            ...
            <div className="grid grid-cols-3 gap-1">
              {aspectRatioOptions.slice(0, 3).map((opt) => (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    updateNodeData<ZImageGeneratorNodeData>(id, { aspectRatio: opt.value as ZImageGeneratorNodeData["aspectRatio"] });
                  }}
                >
                  {opt.label}
                </button>
          ...
          <div>
            ...
            <textarea
              ...
              onPointerDown={(e) => e.stopPropagation()}
              onChange={(e) => {
                updateNodeData<ZImageGeneratorNodeData>(id, { negativePrompt: e.target.value });
              }}
            />
          </div>
...
// 导出 Z-Image 节点
export const ZImageGeneratorNode = memo((props: NodeProps<ZImageGeneratorNode>) => {
  return <ZImageGeneratorBase {...props} />;
});
ZImageGeneratorNode.displayName = "ZImageGeneratorNode";
